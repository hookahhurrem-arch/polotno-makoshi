...
//#region node_modules/.nitro/vite/services/ssr/assets/day-weaver-CtYGWo1K.js
...
function stripHealth(text) {
	return text.split(/\n+/).filter((line) => !/здоров|недуг|болезн|бессон|давлен|фоби|травм/i.test(line)).map((line) => line.replace(/^(В отношениях|В делах и деньгах|В делах|В деньгах|Как человек)\s*[-—:]*\s*/i, "").trim()).filter(Boolean).join(" ");
}
function firstSentence(text) {
	return (text.match(/[^.!?]+[.!?]+/g)?.map((s) => s.trim()) ?? [text.trim()])[0] || text.trim();
}
function coreOf(description, keywords, title) {
	return firstSentence(stripHealth(description)) || keywords.trim() || title.trim();
}
...
function parseReading(raw) {
	...
	try {
		...
		if (/по здоровью|в отношениях —|в делах —|глухая оборона|не замуровывайся|всё обязательно закончится хорошо/i.test(`${dayLine} ${dayLooks} ${dayDo} ${dayAvoid}`)) return null;
		return {
			dayLine,
			dayLooks,
			dayDo,
...
var weaveDayThread_createServerFn_handler = createServerRpc({
	id: "3c5f31336d46197db53890c5199976876134652da4bce22153d894b512ff7ce3",
	name: "weaveDayThread",
	filename: "src/lib/oracle/day-weaver.ts"
}, (opts) => weaveDayThread.__executeServer(opts));
var weaveDayThread = createServerFn({ method: "POST" }).validator((input) => inputSchema.parse(input)).handler(weaveDayThread_createServerFn_handler, async ({ data }) => {
...
Напиши нить дня на сегодня.`
	...
	try {
		...
		const parsed = parseReading((await res.json()).choices?.[0]?.message?.content?.trim() || "");
		if (parsed) return {
			ok: true,
			reading: parsed,
			source: "grok"
...
//#endregion
export { weaveDayThread_createServerFn_handler };
